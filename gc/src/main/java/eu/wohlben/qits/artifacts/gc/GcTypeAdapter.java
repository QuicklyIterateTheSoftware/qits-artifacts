...
public interface GcTypeAdapter {
  ...
  /**
   * The keep-class checked <b>before</b> the access rule: which live pin names one of these
   * identities <b>by coordinate</b>, and null for the ones nothing names.
   *
   * <p>Handed the whole enumeration rather than one candidate at a time, because a coordinate pin is
   * often a fact about a <em>group</em> — "this image's newest build" cannot be answered by looking
   * at one tag — and because reading the rows once per run is what keeps a plan judged against a
   ...
   */
  default GcPinned pinnedBy(List<GcCandidate> candidates, GcPins pins) {
    return GcPinned.NONE;
  }
