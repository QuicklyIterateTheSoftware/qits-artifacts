 ...
 *
 * <p>Public for {@code PackagedProcessIT}'s sake, the treatment {@code docs/TinyBundle} already has:
 * that suite lives in {@code eu.wohlben.qits} and drives the daemon wire in the packaged process, so
 * the alternative is a second synthesiser of the same bytes in a second package.
 *
 ...
 */
public final class TinyDaemon {

  private TinyDaemon() {}

  /** {@code 0x7f E L F}, then {@code salt} repeated to {@code size} bytes. */
  public static byte[] binary(String salt, int size) {
